...
// value_ptr - pre-checked for NULL
jvmtiError
JvmtiEnv::GetHeapInfoBeforeGC(char** value_ptr, jboolean ignore_extended) {
  jvmtiError err = JVMTI_ERROR_NONE;
  stringStream info;
  const char* value;

  Universe::print_heap_before_gc(&info, ignore_extended);
  value = info.as_string();
  if (value == NULL) {
    err =  JVMTI_ERROR_NOT_AVAILABLE;
  } else {
    err = allocate((strlen(value)+1) * sizeof(char), (unsigned char **)value_ptr);
